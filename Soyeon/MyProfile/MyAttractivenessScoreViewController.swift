    ...
    @IBAction private func rescoreButtonTapped(_ sender: UIButton) {
        SoyeonBasicAlertView
            ...
            .action(style: .basic("아이템 충전하기"), completion: { [weak self] _ in
                ...
                self.soyeonRatingView.configure(
                    with: SoyeonRatingViewModel(
                        currentScore: 0.0,
                        levels: [
                            SoyeonRatingViewModel.Level(score: 1.0, title: "브론즈"),
                ...
                self.userStackView.arrangedSubviews.forEach { $0.isHidden = true }
                ...
                self.userStackView.addArrangedSubview(progressLabel)
                self.rescoreButton.isHidden = true
                let guideLabel = UILabel()
                guideLabel.font = Fonts.nanumSquareR.size(12)
                guideLabel.textAlignment = .center
                guideLabel.text = "일정 시간이 소요되니 기다려주세요!"
                guideLabel.textColor = UIColor(red: 102/255, green: 102/255, blue: 102/255, alpha: 1.0)
                guideLabel.translatesAutoresizingMaskIntoConstraints = false
                self.view.addSubview(guideLabel)
                guideLabel.centerXAnchor.constraint(equalTo: self.view.centerXAnchor).isActive = true
                guideLabel.topAnchor.constraint(equalTo: self.soyeonRatingView.bottomAnchor,
